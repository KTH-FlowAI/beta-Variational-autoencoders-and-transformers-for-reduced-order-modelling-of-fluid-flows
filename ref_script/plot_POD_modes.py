...
file = '08_POD/POD_twoPlatesRe100_tc1_Data.h5py'
...
def modesPlot(Phi_x, Phi_y, Psi, Ulim, fs, firstN):
    ...
    for m in range(firstN):
        ...
        annot_max(f / fs, Pxx_den/max(Pxx_den), ax=ax[m, 2], xytext=(0.94, 0.96))
...
fig, ax = modesPlot(phi_u, phi_v, Psi_train, Ulim, fs, firstN)
fig.set_tight_layout(True)
