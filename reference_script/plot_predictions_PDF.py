...
for i, uniqNmode in enumerate(diffNmodes):
    ...
    fig, ax = plt.subplots(1, nplots, figsize=(9, 1.5), sharex='col', sharey='row')
    truePlotted = False
    ...
    lgd = fig.legend(handles, labels, loc="upper center", ncol=4, bbox_to_anchor=(0.5, 1.2))

    plt.savefig(f"05_Figs/vis_pred/PDFpreds_{uniqNmode}.png", bbox_extra_artists=(lgd,), bbox_inches="tight")
    plt.savefig(f"05_Figs/vis_pred/PDFpreds_{uniqNmode}.eps", bbox_extra_artists=(lgd,), format='eps',
                bbox_inches="tight")
